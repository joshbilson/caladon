...
export const parseConvo = ({
  ...
  // Caladon: a CUSTOM endpoint (e.g. "caladon") has no built-in primary schema. Rather than throw
  // "Unknown endpoint" when a caller omits endpointType (some SidePanel/preset flows do — which
  // otherwise breaks chat submit AND the side panels), fall back to the custom endpoint schema.
  const effectiveType = endpointType ?? (!primarySchema ? EModelEndpoint.custom : endpointType);

  const schema =
    primarySchema ??
    getFallbackEndpointSchema(endpointSchemas, effectiveType, defaultParamsEndpoint);
...
export const parseCompactConvo = ({
  ...
  // Caladon: custom endpoint fallback (see parseConvo above) — never throw "Unknown endpoint" for a
  // custom EP that omits endpointType; use the custom endpoint schema so submit + panels don't break.
  const effectiveType = endpointType ?? (!primarySchema ? EModelEndpoint.custom : endpointType);

  const schema =
    primarySchema ??
    getFallbackEndpointSchema(compactEndpointSchemas, effectiveType, defaultParamsEndpoint);
