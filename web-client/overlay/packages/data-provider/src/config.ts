...
/**
 * Caladon surgery (SURGERY.md §B2 / §A2): every chat route collapses to the single same-origin
 * shim opener (`/api/caladon/chat`), which relays to `gw.caladon.ai`'s `POST /v1/chat`. The
 * per-provider/assistant LibreChat server routes are amputated — they no longer exist. The model
 * slug is carried in the body and honoured only if the gateway is attested.
 */
const CALADON_CHAT_URL = '/api/caladon/chat';

export const EndpointURLs = {
  [EModelEndpoint.assistants]: CALADON_CHAT_URL,
  [EModelEndpoint.azureAssistants]: CALADON_CHAT_URL,
  [EModelEndpoint.agents]: CALADON_CHAT_URL,
} as const;
