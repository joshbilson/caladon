...
export default defineConfig(({ command }) => ({
  ...
  server: {
    ...
    strictPort: false,
    // Allow Vite to serve the externally-aliased Caladon SDK + wasm (web-client/caladon),
    // which lives above the client root.
    fs: {
      allow: [path.resolve(__dirname, '..'), path.resolve(__dirname, '../../caladon')],
    },
    proxy: {
      // The Caladon shim (web-client/shim) is the only server the SPA talks to: it relays
      // /api/caladon/* and /pcs-collateral/* to gw.caladon.ai. Point the dev proxy at it.
      '/api/caladon': {
        target: process.env.CALADON_SHIM_URL || 'http://localhost:8787',
        changeOrigin: true,
      },
      '/pcs-collateral': {
        target: process.env.CALADON_SHIM_URL || 'http://localhost:8787',
  ...
  resolve: {
    alias: {
      ...
      // Caladon protocol SDK (SURGERY.md §D INJECT layer). Lives OUTSIDE the vendored
      // LibreChat tree at web-client/caladon/; aliased so the fork imports it as a package.
      // The /wasm subpath alias keeps the `?url` wasm import resolving to the built binary.
      '@caladon/protocol/wasm': path.resolve(__dirname, '../../caladon/wasm'),
      '@caladon/protocol': path.resolve(__dirname, '../../caladon/src/index.ts'),
    },
  },
}));
