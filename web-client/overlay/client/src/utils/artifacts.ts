...
/**
 * Caladon artifact lock-down (SURGERY.md §C2). A remote `script-src` (the public Tailwind CDN) is
 * itself a load + beacon vector and is incoherent with `connect-src 'none'`. Self-host a vendored
 * Tailwind from the SPA origin (`/artifacts/tailwind.js`, served by the shim/static) so the artifact
 * document only ever fetches same-origin. The `#tailwind.js` fragment keeps Sandpack's static
 * template type-detection happy. Provision the file at the deploy step (BLOCKED ON P3 §3 — infra).
 */
const TAILWIND_SELF_HOSTED = '/artifacts/tailwind.js#tailwind.js';

export const sharedOptions: SandpackProviderProps['options'] = {
  externalResources: [TAILWIND_SELF_HOSTED],
};

/**
 * Self-hosted Sandpack bundler (SURGERY.md §C2). The public `*.codesandbox.io` bundler is a
 * remote-fetch/exfil surface and competes with `connect-src 'none'`; serve the bundler from our
 * own origin. `startupConfig.{bundlerURL,staticBundlerURL}` win if set (deploy override); otherwise
 * fall back to the same-origin `/sandpack-bundler/` path.
 */
const SELF_HOSTED_BUNDLER = '/sandpack-bundler/';
...
export function buildSandpackOptions(
  ...
  if (!startupConfig) {
    return { ...sharedOptions, bundlerURL: SELF_HOSTED_BUNDLER };
  }
  ...
  return {
    ...sharedOptions,
    bundlerURL: configured || SELF_HOSTED_BUNDLER,
  };
}
  ...
  '/public/index.html': dedent`
    ...
    <html lang="en">
      <head>
        ...
        <meta http-equiv="Content-Security-Policy" content="default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; connect-src 'none'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:;">
        <title>Document</title>
        <script src="/artifacts/tailwind.js"></script>
        <style>
