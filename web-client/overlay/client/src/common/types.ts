...
export type TAuthContext = {
  ...
  /**
   * Caladon seed identity (SURGERY.md §A3 / §D). Present after a fail-closed handshake. The seed
   * never leaves the device; `accountId` is the key-bound routing id; `signRequest` produces the
   * per-request `Swifty …` Authorization header from the in-WASM signer.
   */
  caladon?: {
    accountId: string | undefined;
    isUnlocked: boolean;
    /** Run the full fail-closed handshake from a 32-byte seed. Throws on attestation failure. */
    unlock: (seed: Uint8Array) => Promise<void>;
    lock: () => void;
    /** Per-request signed header for an upstream `/v1/...` gateway path. */
    signRequest: (method: 'GET' | 'POST', gatewayPath: string) => Promise<string>;
  };
};
